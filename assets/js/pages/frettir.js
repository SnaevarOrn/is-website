...
(() => {
  ...
  const SOURCES = [
    ...
    { id: "skagafjordur",  label: "Skagafjörður",    domain: "skagafjordur.is" },
  ...
  const LIMIT_PER_BATCH = 50;    // items per batch request
  ...
  sources: Object.fromEntries(SOURCES.map(s => [
    s.id,
    ["ruv", "mbl", "bb", "bbl", "vb", "fiskifrettir", "visir", "dv"].includes(s.id)
  ])),
  categories: Object.fromEntries(CATEGORIES.map(c => [
    c.id,
    !["ithrottir", "oflokkad"].includes(c.id)
  ])),
});
