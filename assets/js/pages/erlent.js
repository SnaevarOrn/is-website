...
(() => {
  ...
  const STORAGE_KEY = "is_worldnews_prefs_v1";

  // Read/visited tracking
  const READ_KEY = "is_worldnews_read_v1";
  ...
  async function refresh() {
    ...
    try {
      ...
      console.error("[erlent] refresh error", err);
  ...
  async function showReadingView(url) {
    ...
    try {
      ...
      console.error("[erlent] readingview error", err);
